    ...
    grid.innerHTML = data.sort((a, b) => {
        ...
        return `
            <div class="card ${tier}" ${clickAttr} style="cursor: ${tier !== 'basic' ? 'pointer' : 'default'};">
                ...
                ${tier === 'plus' ? `<div class="plus-reveal"><p><strong>Phone:</strong> ${biz.Phone}</p><p><strong>Est:</strong> ${biz['Date Started'] || 'N/A'}</p></div>` : ''}
                <div style="margin-top: auto; font-style: italic; font-size: 0.85rem; color: #444;">${catEmojis[category] || "🏭"} ${category}</div>
            </div>`;
    }).join('');
}

// 8. PROFILE PAGE ENGINE
